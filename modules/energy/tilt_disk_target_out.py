"""Outer-leaflet disk tilt target energy (soft profile enforcement).

This module applies a soft penalty that drives the outer-leaflet tilt field
to a prescribed radial profile over a tagged disk:

    E = 1/2 * k * ∫ |t_out - theta(r) r_hat|^2 dA
...
Parameters
----------
- tilt_disk_target_group_out: group name (string; required).
- tilt_disk_target_strength_out: k (float; default 0).
- tilt_disk_target_theta_B_out: boundary tilt at r=R (float; optional; falls back
  to tilt_disk_target_theta_B).
- tilt_disk_target_lambda_out: lambda (float; optional; computed from moduli if absent).
- tilt_disk_target_center_out: 3D center point (default [0,0,0]).
- tilt_disk_target_normal_out: plane normal (optional; fit from disk if absent).
- tilt_disk_target_radius_out: R (float; optional; uses max disk radius if absent).
"""
...
def _resolve_group(param_resolver) -> str | None:
    raw = param_resolver.get(None, "tilt_disk_target_group_out")
    if raw is None:
        return None
    group = str(raw).strip()
...
def _resolve_strength(param_resolver) -> float:
    val = param_resolver.get(None, "tilt_disk_target_strength_out")
    return float(val or 0.0)


def _resolve_theta_b(param_resolver) -> float:
    val = param_resolver.get(None, "tilt_disk_target_theta_B_out")
    if val is None:
...
def _resolve_center(param_resolver) -> np.ndarray:
    center = param_resolver.get(None, "tilt_disk_target_center_out")
    if center is None:
...
def _resolve_normal(param_resolver) -> np.ndarray | None:
    raw = param_resolver.get(None, "tilt_disk_target_normal_out")
    if raw is None:
...
def _resolve_radius(param_resolver) -> float | None:
    raw = param_resolver.get(None, "tilt_disk_target_radius_out")
    if raw is None:
...
def _resolve_lambda(param_resolver) -> float:
    raw = param_resolver.get(None, "tilt_disk_target_lambda_out")
    if raw is None:
    ...
    k_tilt = param_resolver.get(None, "tilt_modulus_out")
    kappa = param_resolver.get(None, "bending_modulus_out")
    if kappa is None:
...
def _collect_group_rows(mesh: Mesh, group: str) -> np.ndarray:
    ...
    for vid in mesh.vertex_ids:
        ...
        if opts.get("tilt_disk_target_group_out") == group:
...
def compute_energy_and_gradient_array(
    ...
    *,
    ...
    target = np.zeros_like(tilts_out)
    target[disk_rows] = theta[:, None] * r_hat

    diff = tilts_out - target
    ...
    if tilt_out_grad_arr is not None:
        ...
        tilt_out_grad_arr += k_target * diff * vertex_areas[:, None]
