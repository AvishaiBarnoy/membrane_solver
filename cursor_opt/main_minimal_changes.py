...
def parse_instructions(instr):
    ...
    for inst in instr:
        ...
        if cmd.startswith('g'):
            ...
            result.append('gd')
        elif cmd.startswith("visualize" or cmd == "s"):
            result.append(cmd)
...
def main():
    ...
    # ⚡ APPLY OPTIMIZATIONS (NEW - JUST ONE LINE!)
    patch_optimizations()
    if not args.quiet:
    ...
    # ⚡ USE OPTIMIZED MINIMIZER (CHANGED - JUST ONE LINE!)
    minimizer = OptimizedMinimizer(mesh, global_params, stepper, energy_manager,
                          constraint_manager, quiet=args.quiet, use_optimizations=True)
