...
def _disk_fan_mesh(*, n: int = 8, radius: float = 1.0) -> dict:
    ...
    vertices: list[list] = []
    vertices.append([0.0, 0.0, 0.0, {"tilt_disk_target_group_in": "disk"}])
    for i in range(n):
        theta = 2.0 * np.pi * i / n
        vertices.append(
            [
    ...
    return {
        "global_parameters": {
            "tilt_disk_target_group_in": "disk",
            "tilt_disk_target_strength_in": 10.0,
...
def test_tilt_disk_target_zero_when_matching() -> None:
    from modules.energy import tilt_disk_target_in
    ...
    tilts_in = theta[:, None] * r_hat
    mesh.set_tilts_in_from_array(tilts_in)

    energy = tilt_disk_target_in.compute_energy_and_gradient_array(
        mesh,
...
def test_tilt_disk_target_penalizes_mismatch() -> None:
    from modules.energy import tilt_disk_target_in
    ...
    mesh.set_tilts_in_from_array(np.zeros_like(positions))
...
def test_tilt_disk_target_energy_array_matches_gradient_path() -> None:
    from modules.energy import tilt_disk_target_in
    ...
    tilts_in = mesh.tilts_in_view()
    e_grad = tilt_disk_target_in.compute_energy_and_gradient_array(
        mesh,
        mesh.global_parameters,
        resolver,
        positions=positions,
    ...
    e_only = tilt_disk_target_in.compute_energy_array(
        mesh,
        mesh.global_parameters,
        resolver,
        positions=positions,
