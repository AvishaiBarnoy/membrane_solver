...
def test_global_parameters_loading():
    ...
    assert isinstance(mesh.global_parameters, GlobalParameters), (
        "global_parameters should be an instance of GlobalParameters"
    )

    # Check that predefined parameters are updated correctly
    assert mesh.global_parameters.get("surface_tension") == 10.0, (
        "surface_tension should be updated to 10.0"
    )
    assert mesh.global_parameters.get("volume_stiffness") == 500.0, (
        "volume_stiffness should be updated to 500.0"
    )

    # Check that user-defined parameters are added
    assert mesh.global_parameters.get("custom_param") == 42.0, (
        "custom_param should be added with value 42.0"
    )

    # Check that default parameters are preserved
    assert mesh.global_parameters.get("step_size") == 1e-3, (
        "step_size should retain its default value"
    )
...
def test_global_parameters_defaults():
    ...
    assert gp.get("volume_stiffness") == 1000.0, (
        "Default volume_stiffness should be 1000.0"
    )
...
def test_global_parameters_update():
    ...
    gp.update({"surface_tension": 15.0, "new_param": 99.0})

    # Check updated values
    assert gp.get("surface_tension") == 15.0, (
        "surface_tension should be updated to 15.0"
    )
    assert gp.get("new_param") == 99.0, "new_param should be added with value 99.0"

    # Check that other defaults are preserved
    assert gp.get("volume_stiffness") == 1000.0, (
        "volume_stiffness should retain its default value"
    )
