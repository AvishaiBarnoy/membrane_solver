...
def test_flat_disk_curved_3d_ablation_sweep_returns_ranked_candidates() -> None:
    sweep_cfg = yaml.safe_load(
        (
            ...
            / "tests"
            / "fixtures"
            / "flat_disk_curved_3d_ablation_sweep_smoke.yaml"
        ).read_text(encoding="utf-8")
    )
    report = run_flat_disk_curved_3d_ablation_sweep(
        fixture=ROOT
        / "tests"
        / "fixtures"
    ...
    assert report["meta"]["mode"] == "curved_3d_ablation_sweep_smoke"
    ...
    assert np.isfinite(float(row["theta_factor_observed"]))
    assert np.isfinite(float(row["energy_factor_observed"]))
    assert np.isfinite(float(row["theta_star_mesh"]))
    assert np.isfinite(float(row["total_energy_mesh"]))
