...
def test_flat_disk_kh_error_source_rank_effects_orders_descending() -> None:
    ranked = _rank_effects(
        {"mass_effect": 0.1, "resolution_effect": 0.2, "operator_effect": 0.05}
    )
    assert ranked["dominant_source"] == "resolution_effect"
    assert ranked["ranking"] == ["resolution_effect", "mass_effect", "operator_effect"]
...
def test_flat_disk_kh_error_source_audit_emits_finite_rows(
            ...
            "rows": [
                {
                    ...
                    "section_score_internal_bands_finite_outer_l2_log": float(
                        np.hypot(np.log(near), np.log(max(far, 1e-18)))
                    ),
    ...
    report = run_flat_disk_kh_error_source_audit(
        refine_levels=(2, 3), mass_modes=("consistent", "lumped")
    )
