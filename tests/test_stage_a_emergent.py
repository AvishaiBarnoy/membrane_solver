from __future__ import annotations

from pathlib import Path

import numpy as np
import pytest
from scipy import special

from commands.context import CommandContext
from commands.executor import execute_command_line
from geometry.geom_io import load_data, parse_geometry
from tools.build_stage_a_fixtures import (
    BASE_FIXTURE,
    SEEDED_FIXTURE,
    STAGE_A_RIM_SOURCE_STRENGTH,
)
from tools.diagnostics.flat_disk_one_leaflet_theory import (
    compute_flat_disk_theory,
    tex_reference_params,
)
from tools.reproduce_flat_disk_one_leaflet import _build_minimizer

TARGET_EMERGENT_THETA = 0.18
EMERGENT_Z_THRESHOLD = 5.0e-5
EMERGENT_THETA_THRESHOLD = 1.0e-2
PROFILE_RMSE_MAX = 1.2


def _load_mesh_from_fixture(path: Path):
    return parse_geometry(load_data(str(path)))


def _disk_boundary_rows(mesh) -> np.ndarray:
    rows: list[int] = []
    for vid in mesh.vertex_ids:
        opts = getattr(mesh.vertices[int(vid)], "options", None) or {}
        if opts.get("rim_slope_match_group") != "disk":
            continue
        rows.append(int(mesh.vertex_index_to_row[int(vid)]))
    return np.asarray(rows, dtype=int)


def _disk_rows(mesh) -> np.ndarray:
    rows: list[int] = []
    for vid in mesh.vertex_ids:
        opts = getattr(mesh.vertices[int(vid)], "options", None) or {}
        if str(opts.get("preset") or "") != "disk":
            continue
        rows.append(int(mesh.vertex_index_to_row[int(vid)]))
    return np.asarray(rows, dtype=int)


def _radial_frame(positions: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    r_vec = np.array(positions, copy=True)
    r_vec[:, 2] = 0.0
    r_len = np.linalg.norm(r_vec, axis=1)
    r_hat = np.zeros_like(r_vec)
    valid = r_len > 1.0e-12
    r_hat[valid] = r_vec[valid] / r_len[valid, None]
    return r_len, r_hat


def _measure_state(mesh, minimizer) -> dict[str, float | int | str]:
    positions = mesh.positions_view()
    tilts_sum = mesh.tilts_in_view() + mesh.tilts_out_view()

    boundary_rows = _disk_boundary_rows(mesh)
    boundary_r, boundary_hat = _radial_frame(positions[boundary_rows])
    boundary_theta = np.sum(tilts_sum[boundary_rows] * boundary_hat, axis=1)
    boundary_valid = boundary_r > 1.0e-12

    disk_rows = _disk_rows(mesh)
    disk_r, disk_hat = _radial_frame(positions[disk_rows])
    disk_theta = np.sum(tilts_sum[disk_rows] * disk_hat, axis=1)
    disk_valid = disk_r > 1.0e-12

    profile_rmse = float("nan")
    if np.any(disk_valid) and np.any(boundary_valid):
        r_disk = disk_r[disk_valid]
        theta_disk = disk_theta[disk_valid]
        order = np.argsort(r_disk)
        r_disk = r_disk[order]
        theta_disk = theta_disk[order]
        r_unique = np.unique(np.round(r_disk, 6))
        pairs: list[tuple[float, float]] = []
        for radius in r_unique:
            mask = np.isclose(r_disk, radius, atol=1.0e-6)
            pairs.append((float(radius), float(np.mean(theta_disk[mask]))))
        if pairs:
            radius_max = max(radius for radius, _ in pairs)
            theta_boundary = max(value for radius, value in pairs if radius > 0.4)
            lambda_value = float(np.sqrt(1.0 / 225.0))
            errors: list[float] = []
            for radius, value in pairs:
                if radius <= 1.0e-8 or abs(theta_boundary) <= 1.0e-12:
                    continue
                ref = float(
                    special.iv(1, radius / lambda_value)
                    / special.iv(1, radius_max / lambda_value)
                )
                errors.append(float((value / theta_boundary) - ref))
            if errors:
                profile_rmse = float(np.sqrt(np.mean(np.square(errors))))

    theta_mean = float(np.mean(boundary_theta[boundary_valid]))
    theta_std = float(np.std(boundary_theta[boundary_valid]))
    zmax = float(np.max(np.abs(positions[:, 2])))
    total_energy = float(minimizer.compute_energy())

    branch = "emergent_curved"
    if theta_mean <= EMERGENT_THETA_THRESHOLD and zmax <= EMERGENT_Z_THRESHOLD:
        branch = "flat_control"

    return {
        "branch": branch,
        "theta_mean": theta_mean,
        "theta_std": theta_std,
        "zmax": zmax,
        "total_energy": total_energy,
        "profile_rmse": profile_rmse,
        "vertex_count": int(len(mesh.vertex_ids)),
    }


def _assert_branch_metrics(metrics: dict[str, float | int | str]) -> None:
    theory = compute_flat_disk_theory(tex_reference_params())
    if metrics["branch"] == "flat_control":
        assert float(metrics["theta_mean"]) == pytest.approx(
            float(theory.theta_star), rel=0.5, abs=5.0e-4
        )
        assert float(metrics["total_energy"]) == pytest.approx(
            float(theory.total), rel=0.5, abs=5.0e-3
        )
        assert float(metrics["zmax"]) <= EMERGENT_Z_THRESHOLD
        return

    assert float(metrics["theta_mean"]) == pytest.approx(
        TARGET_EMERGENT_THETA, abs=0.08
    )
    assert float(metrics["theta_mean"]) > EMERGENT_THETA_THRESHOLD
    assert float(metrics["theta_std"]) < 1.0e-2
    assert float(metrics["zmax"]) > EMERGENT_Z_THRESHOLD
    assert np.isfinite(float(metrics["profile_rmse"]))
    assert float(metrics["profile_rmse"]) < PROFILE_RMSE_MAX


def _run_commands(ctx: CommandContext, commands: list[str]) -> None:
    for command in commands:
        execute_command_line(ctx, command)


def _run_fixture(path: Path, commands: list[str]) -> dict[str, float | int | str]:
    mesh = _load_mesh_from_fixture(path)
    minimizer = _build_minimizer(mesh)
    ctx = CommandContext(mesh, minimizer, minimizer.stepper)
    _run_commands(ctx, commands)
    return _measure_state(ctx.mesh, minimizer)


def _run_continuation(path: Path) -> dict[str, float | int | str]:
    mesh = _load_mesh_from_fixture(path)
    minimizer = _build_minimizer(mesh)
    ctx = CommandContext(mesh, minimizer, minimizer.stepper)
    for strength in (0.125, 0.25, 0.375, STAGE_A_RIM_SOURCE_STRENGTH):
        mesh.global_parameters.set("tilt_rim_source_strength", float(strength))
        execute_command_line(ctx, "g3")
    return _measure_state(ctx.mesh, minimizer)


@pytest.mark.acceptance
def test_stage_a_base_fixture_classifies_expected_branch() -> None:
    metrics = _run_fixture(BASE_FIXTURE, ["g5"])
    _assert_branch_metrics(metrics)


@pytest.mark.acceptance
def test_stage_a_seeded_fixture_classifies_expected_branch() -> None:
    metrics = _run_fixture(SEEDED_FIXTURE, ["g5"])
    _assert_branch_metrics(metrics)


@pytest.mark.acceptance
def test_stage_a_continuation_prefers_emergent_branch_when_available() -> None:
    base_metrics = _run_fixture(BASE_FIXTURE, ["g1"])
    continuation_metrics = _run_continuation(BASE_FIXTURE)
    _assert_branch_metrics(continuation_metrics)

    if base_metrics["branch"] == "emergent_curved":
        assert continuation_metrics["branch"] == "emergent_curved"
        assert float(continuation_metrics["theta_mean"]) >= (
            float(base_metrics["theta_mean"]) - 2.0e-2
        )
    else:
        assert float(continuation_metrics["theta_mean"]) >= float(
            base_metrics["theta_mean"]
        )


@pytest.mark.acceptance
def test_stage_a_refinement_keeps_branch_consistent() -> None:
    coarse_metrics = _run_fixture(BASE_FIXTURE, ["g5"])
    refined_metrics = _run_fixture(BASE_FIXTURE, ["g3", "r", "g3"])

    if coarse_metrics["branch"] == "flat_control":
        assert refined_metrics["branch"] == "flat_control"
        _assert_branch_metrics(refined_metrics)
        return

    assert refined_metrics["branch"] == "emergent_curved"
    assert int(refined_metrics["vertex_count"]) > int(coarse_metrics["vertex_count"])
    assert float(refined_metrics["theta_mean"]) > EMERGENT_THETA_THRESHOLD
    assert float(refined_metrics["theta_std"]) < 1.0e-2
    assert np.isfinite(float(refined_metrics["profile_rmse"]))
    assert float(refined_metrics["zmax"]) < 1.0e-5
    assert float(refined_metrics["profile_rmse"]) < 1.5
