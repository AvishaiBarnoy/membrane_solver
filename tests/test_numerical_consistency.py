...
def test_bending_gradient_consistency():
    """Verify bending energy gradient directionality."""
    mesh = create_random_mesh()
    # The bi-Laplacian is a stable approximation, not an exact symbolic derivative
    # of the discrete sum. We check for order-of-magnitude consistency.
    check_gradient_consistency(bending, mesh, eps=1e-4, tol=2.0)


def test_bending_flow_reduces_energy():
    """Verify that taking a small step along the analytical bending force reduces bending energy."""
    mesh = create_random_mesh()
    gp = GlobalParameters()
    gp.bending_modulus = 1.0
    resolver = ParameterResolver(gp)
