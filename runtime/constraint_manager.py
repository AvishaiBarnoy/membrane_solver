...
class ConstraintModuleManager:
    ...
    def apply_joint_gradient_modifications_array(
        ...
        """Project shape and leaflet gradients onto a unified constraint manifold.

        This is used for coupled solve modes where constraints connect vertex
        positions and tilt fields (e.g. rim slope matching).

        Modules providing joint constraints should implement:
            constraint_gradients_joint_array(...) -> list[tuple[Part, Part, Part]]
        where Part is:
            None | np.ndarray | (rows, vectors)
        representing (shape, tilt_in, tilt_out) components of one constraint row.
        """
        mesh.build_position_cache()
        positions = mesh.positions_view()
        index_map = mesh.vertex_index_to_row
        ...
        # 1. Collect all joint constraint rows
        joint_rows: list[
            tuple[
                tuple[np.ndarray, np.ndarray] | None,  # shape (rows, vecs)
        ...
        for module in self.modules.values():
            if not hasattr(module, "constraint_gradients_joint_array"):
                # For non-joint modules, we can still participate if they provide
                # standard separate gradients, but we must treat them as uncoupled
                # rows in the big KKT solve.
                if hasattr(module, "constraint_gradients_rows_array"):
                    g_list = module.constraint_gradients_rows_array(
                        mesh, global_params, positions=positions, index_map=index_map
                    )
                    if g_list:
                        for payload in g_list:
                            r, v = _as_row_gradient_payload(payload)
                            joint_rows.append(((r, v), None, None))

                if hasattr(module, "constraint_gradients_tilt_rows_array"):
                    g_list = module.constraint_gradients_tilt_rows_array(
                        mesh,
                        global_params,
                        positions=positions,
                        index_map=index_map,
                    ...
                    if g_list:
                        for tin_payload, tout_payload in g_list:
                            in_norm = None
                            out_norm = None
                            if tin_payload is not None:
                                r, v = _as_row_gradient_payload(tin_payload)
                                if r.size:
                                    in_norm = (r, v)
                            if tout_payload is not None:
                                r, v = _as_row_gradient_payload(tout_payload)
                                if r.size:
                                    out_norm = (r, v)
                            if in_norm is not None or out_norm is not None:
                                joint_rows.append((None, in_norm, out_norm))
                continue

            # Joint-aware module
            res = module.constraint_gradients_joint_array(
                mesh,
                global_params,
                positions=positions,
                index_map=index_map,
            ...
            for s_part, tin_part, tout_part in res:
                s_norm = None
                in_norm = None
                out_norm = None
                if s_part is not None:
                    r, v = _as_row_gradient_payload(s_part)
                    r, v = _coalesce_sparse_row_payload(r, v)
                    if r.size:
                        s_norm = (r, v)
                if tin_part is not None:
                    r, v = _as_row_gradient_payload(tin_part)
                    r, v = _coalesce_sparse_row_payload(r, v)
                    if r.size:
                        in_norm = (r, v)
                if tout_part is not None:
                    r, v = _as_row_gradient_payload(tout_part)
                    r, v = _coalesce_sparse_row_payload(r, v)
                    if r.size:
                        out_norm = (r, v)
                if s_norm is not None or in_norm is not None or out_norm is not None:
                    joint_rows.append((s_norm, in_norm, out_norm))

        if not joint_rows:
            return

        k = len(joint_rows)
        # Unified vector length: shape + tin + tout
        # We don't actually concatenate the full gradients unless k is large.
        # Instead, we compute b_i = J_i . G and A_ij = J_i . J_j directly.

        b = np.zeros(k, dtype=float)
        A = np.zeros((k, k), dtype=float)

        for i, (s_i, in_i, out_i) in enumerate(joint_rows):
            # Compute b[i] = J_i . G
            val = 0.0
            if s_i is not None:
                r, v = s_i
                val += float(np.sum(grad_arr[r] * v))
            if in_i is not None:
                r, v = in_i
                val += float(np.sum(tilt_in_grad_arr[r] * v))
            if out_i is not None:
                r, v = out_i
                val += float(np.sum(tilt_out_grad_arr[r] * v))
            b[i] = val

            # Compute A[i, j] = J_i . J_j
            for j in range(i, k):
                s_j, in_j, out_j = joint_rows[j]
                dot = 0.0
                if s_i is not None and s_j is not None:
                    # Sparse dot product
                    # We can use a simple approach since constraints are usually localized
                    r_i, v_i = s_i
                    r_j, v_j = s_j
                    # Fast path for identical row sets
                    if r_i.shape == r_j.shape and np.all(r_i == r_j):
                        dot += float(np.sum(v_i * v_j))
                    else:
                        # Fallback: find intersection
                        # For very large constraints this might be slow,
                        # but typically constraints touch few vertices.
                        mask_i = np.isin(r_i, r_j)
                        if np.any(mask_i):
                            idx_j = np.searchsorted(r_j, r_i[mask_i])
                            # (Note: searchsorted assumes r_j is sorted)
                            # Most modules return sorted rows via _collect_group_rows.
                            dot += float(np.sum(v_i[mask_i] * v_j[idx_j]))

                if in_i is not None and in_j is not None:
                    r_i, v_i = in_i
                    r_j, v_j = in_j
                    if r_i.shape == r_j.shape and np.all(r_i == r_j):
                        dot += float(np.sum(v_i * v_j))
                ...
                if out_i is not None and out_j is not None:
                    r_i, v_i = out_i
                    r_j, v_j = out_j
                    if r_i.shape == r_j.shape and np.all(r_i == r_j):
                        dot += float(np.sum(v_i * v_j))
        ...
        # 3. Update gradients: G <- G - J^T * lam
        for i, (s_i, in_i, out_i) in enumerate(joint_rows):
            l_i = lam[i]
            if l_i == 0.0:
                continue
            if s_i is not None:
                r, v = s_i
                np.add.at(grad_arr, r, -l_i * v)
            if in_i is not None:
                r, v = in_i
                np.add.at(tilt_in_grad_arr, r, -l_i * v)
            if out_i is not None:
                r, v = out_i
                np.add.at(tilt_out_grad_arr, r, -l_i * v)
