...
from geometry.polygon_triangulation import (
    orient_edges_cycle,
    refine_polygonal_facets,  # noqa: F401 - legacy public re-export
)
...
def refine_triangle_mesh(mesh):
    ...
    def _merge_constraints(options: dict, additions: list[str]) -> None:
        merge_constraints_in_place(options, additions)

    def _has_fixed_constraint(options: dict | None) -> bool:
        return has_fixed_constraint(options)

    def _apply_preset_definitions(options: dict) -> tuple[dict, bool]:
        return apply_preset_definitions(options, getattr(mesh, "definitions", {}) or {})
    ...
    def _maybe_inherit_disk_interface_vertex_tags(
        ...
        return inherit_disk_interface_tags(v1_options, v2_options)

    def _maybe_inherit_rigid_disk_group(
        v1_options: dict, v2_options: dict
    ) -> dict | None:
        """Inherit rigid-disk group when both endpoints share it."""
        return inherit_rigid_disk_group(v1_options, v2_options)

    def _maybe_inherit_preset(
        v1_options: dict, v2_options: dict
    ) -> tuple[str | None, bool]:
        """Return a deterministic preset and whether to apply preset defaults."""
        return choose_midpoint_preset(
            v1_options, v2_options, getattr(mesh, "definitions", {}) or {}
        )

    def _is_ring_like_preset(preset: object) -> bool:
        return is_ring_like_preset(preset, getattr(mesh, "definitions", {}) or {})
