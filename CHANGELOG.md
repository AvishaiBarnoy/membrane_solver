...
- Flat one-leaflet disk benchmark reproduction tooling:
  ...
  - Flat disk optimize presets now include opt-in `kh_strict_fast` for strict-KH runs with reduced optimize work while preserving strict mesh refinement controls.
  - `kh_strict_fast` was retuned for faster strict-KH runs (`optimize_steps=30`, `optimize_delta=6e-3`) while keeping strict parity stable; optimize reports now include completed-iteration and plateau-stop telemetry.
...
- Added opt-in `kh_strict_outerband_tight` preset (strict refine=2, rim-local step=1, band=3λ, energy-focused polish) and benchmark coverage that enforces explicit outer-near section-ratio improvement while guarding inner/global parity vs `kh_strict_section_tight`.
...
- Added opt-in outer-annulus local vertex-averaging controls to flat-disk KH tooling:
  ...
  (in both `tools/reproduce_flat_disk_one_leaflet.py` and
  `tools/diagnostics/flat_disk_kh_term_audit.py`).
- Added opt-in `kh_strict_outerfield_averaged` preset
  ...
  - KH term audit now reports fixed-theta per-term regional decomposition (tilt vs splay across disk-core/rim-band/outer-near/outer-far) with mesh/theory ratio fields for isolating internal-energy mismatch sources.
...
- KH term audit now reports per-band leakage (`t_phi/t_r`) and anisotropy metrics (`h_max/h_min`, edge-orientation spread) plus cross-band correlations.
  ...
  - Region-parity diagnostics now support strict rim-local sweep candidates (`--rim-local-refine-steps`, `--rim-local-refine-band-lambdas`) with deterministic best-candidate ranking by internal partition score then runtime.
